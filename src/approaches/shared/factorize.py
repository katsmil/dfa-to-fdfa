...
#         # If the start_node was accepting in the original graph, the RC node
#         # that replaces it must also be accepting — it fully takes its place.
#         rc_shape = 'doublecircle' if _is_accepting_node(G, loc.start_node) else 'box'
#         G.add_node(rc_id, shape=rc_shape, style='filled', fillcolor='orange',
#                    label=f"RC: {sub_name}")
