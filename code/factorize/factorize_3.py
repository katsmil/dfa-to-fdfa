        ...
        for pair in frontiers:
            ...
            while queue:
                ...
                for _, v, data in out_edges:
                    ...
                    if v == slave_start:
                        # Dan is voldoende om RC een terugkoppeling/self-loop/return te geven:
                        # dat betekent: "terug naar context" en we hoeven die tak niet verder te reconstrueren.
                        print(f"    - Tak van {u} gaat naar slave_start {slave_start}: voeg self-loop RC -> RC toe (label={data.get('label','?')})")
                        G.add_edge(rc_node_id, rc_node_id, **(data or {}))
                        continue
                    ...
                    # Nu: v is binnen slave_nodes. Controleer of master dit interne pad afdekt.
                    mu = slave_to_master_map.get(u)
                    mv = slave_to_master_map.get(v)
                    master_covers = False

                    if mu is not None and mv is not None:
                        # Kijk in de huidige graaf of mu->mv met dezelfde signatuur bestaat
                        for _, w, mdata in G.out_edges(mu, data=True):
                            if w == mv and _edge_signature(mdata) == sig:
                                master_covers = True
                                break

                    if master_covers:
                        # Master dekt deze overgang af: geen preservatie nodig
                        # (we vertrouwen de CALL naar master om dit intern af te handelen)
                        continue

                    # Master dekt NIET: we moeten deze interne knoop en transitie preserven (klonen onder RC)
                    if v not in preserved_map:
                        new_v = f"{rc_node_id}_{v}"
                        # Kopieer node-attributes veilig (als node bestaat)
                        node_attrs = dict(G.nodes[v]) if v in G.nodes else {}
                        # Zorg dat de nieuwe naam uniek is; als die al bestaat voeg suffix toe
                        if new_v in G.nodes:
                            suffix = 1
                            while f"{new_v}_{suffix}" in G.nodes:
                                suffix += 1
                            new_v = f"{new_v}_{suffix}"
                        G.add_node(new_v, **node_attrs)
                        preserved_map[v] = new_v
                        print(f"      - Gekloond: {v} -> {new_v}")

                    # Bepaal bron voor de nieuwe edge: als u ook gekloond is, gebruik die; anders RC
                    src_for_edge = preserved_map.get(u, rc_node_id)
                    tgt_for_edge = preserved_map[v]

                    # Voeg edge toe tussen gekloonde nodes (of vanaf RC)
                    print(f"      - Voeg gekloonde edge {src_for_edge} -> {tgt_for_edge} (label={data.get('label','?')})")
                    G.add_edge(src_for_edge, tgt_for_edge, **(data or {}))

                    # Blijf verkennen vanaf v (om diepgaand gedrag te detecteren)
                    if v not in visited:
                        queue.append(v)
