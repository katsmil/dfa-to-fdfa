...
class SubstructureAnalyzer:
    ...
    def find_maximal_overlap(self, start_a: str, start_b: str) -> Optional[SubstructureMatch]:
        ...
        # 3. Frontier Detectie & Determinisme Check (één kant is genoeg)
        internals: Set[Tuple[str, str]] = set()
        frontiers: Set[Tuple[str, str]] = set()
        exits_a: Dict[str, str] = {} # label -> target_node_outside

        for p in bisimilar_pairs:
            n1, _ = p
            edges = DFAUtils.get_labeled_edges(self.G, n1)
            is_frontier = False

            for label, target in edges.items():
                # Check of de transitie de substructuur verlaat
                if target not in nodes_in_a:
                    is_frontier = True
                    
                    # NON-DETERMINISME CHECK:
                    # Als dit label al eens naar buiten wees dan hebben we een probleem van NDM.
                    if label in exits_a and exits_a[label] != target:
                        # We hebben twee verschillende frontier-nodes die met hetzelfde label 
                        # naar verschillende plekken buiten de structuur wijzen.
                        return None 
